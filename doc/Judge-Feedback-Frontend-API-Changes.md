...
4. 如果判题状态为非 AC：
   - 若提交详情中的 `feedback` 已有值，直接展示。
   - 若 `feedback` 为 `null`，轮询 `GET /submission/{id}/feedback`。
...
- `feedback`：智能判题反馈，尚未生成或 AC 提交时为 `null`。
...
反馈尚未生成、AC 提交无反馈或提交不存在时：

```json
{
  "code": 404,
  "message": "Submission feedback does not exist",
  "data": null
}
```
...
| `STATIC_FALLBACK` | `STATIC_FALLBACK` | 正常展示 `content`，可标记为“判题提示” |
...
- `STATIC_FALLBACK` 是正常降级结果，不应展示为系统报错。
...
export interface JudgeFeedback {
  ...
  status: 'SUCCESS' | 'STATIC_FALLBACK' | string
  content: string | null
  source: 'LLM' | 'STATIC_FALLBACK' | string | null
  model: string | null
